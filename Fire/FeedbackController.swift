...
class FeedbackController: UIViewController, UITextViewDelegate {
    ...
    let subTitleLabel: UILabel = {
        ...
        label.numberOfLines = 0
        label.sizeToFit()
        label.text = "의견을 보내주시면 검토하여 서비스가 개선될 수 있도록 노력하겠습니다."
    ...
    lazy var saveFeedbackButton: UIButton = {
        let button = UIButton(type: .system)
        button.setTitle("Save", for: .normal)
        button.setTitleColor(.lightGray, for: .normal)
        button.translatesAutoresizingMaskIntoConstraints = false
        button.addTarget(self, action: #selector(saveFeedbackButtonDidTap), for: .touchUpInside)
    ...
    override func viewDidLoad() {
        ...
        navigationItem.rightBarButtonItem = UIBarButtonItem(title: "Save", style: UIBarButtonItemStyle.plain, target: self, action: #selector(saveFeedbackButtonDidTap))
    ...
    func setupTextViews() {
        ...
        feedbackTextView.anchor(top: containerView.topAnchor, left: containerView.leftAnchor, bottom: containerView.bottomAnchor, right: containerView.rightAnchor, paddingTop: 0, paddingLeft: 0, paddingBottom: 0, paddingRight: 0, width: 0, height: 0) // left 8
    ...
    @objc func saveFeedbackButtonDidTap() {

        guard let uid = Auth.auth().currentUser?.uid else { return }
        guard let feedback = feedbackTextView.text, feedback.count > 0 else { return }
        
        let ref = Database.database().reference().child("feedback").child(uid).childByAutoId()
        let value = ["text": feedback, "createdAt": Date().timeIntervalSince1970] as [String: Any]
        ref.updateChildValues(value) { (err, ref) in
            ...
            print("successfully saved to feedback")
